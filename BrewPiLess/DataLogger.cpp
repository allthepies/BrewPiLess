...
Number.prototype.format = function (n, x, s, c) {
	...
	return (c ? num.replace('.', c) : num).replace(new RegExp(a, 'g'), '$&' + (s || ','))
};
String.prototype.escapeJSON = function () {
	return this.replace(/[\\]/g, '\\\\').replace(/[\"]/g, '\\\"').replace(/[\/]/g, '\\/').replace(/[\b]/g, '\\b').replace(/[\f]/g, '\\f').replace(/[\n]/g, '\\n').replace(/[\r]/g, '\\r').replace(/[\t]/g, '\\t')
};
var logs = {
	url: "loglist.php",
	...
	list: function (l) {
		var e = EI("loglist").querySelector("tbody");
		var f;
		while (f = e.querySelector("tr:nth-of-type(2)"))
			e.removeChild(f);
		var t = this;
		var g = t.row;
		l.forEach(function (i, a) {
			var b = i.name;
			var c = new Date(i.time * 1000);
			var d = g.cloneNode(true);
			d.querySelector(".logid").innerHTML = b;
			d.querySelector(".logdate").innerHTML = c.toLocaleString();
			d.querySelector(".dlbutton").onclick = function () {
				t.dl(a)
			};
			d.querySelector(".viewbutton").onclick = function () {
				t.view(a)
			};
			d.querySelector(".rmbutton").onclick = function () {
				t.rm(a)
			};
			e.appendChild(d)
		})
...
function checkformat(a) {
	if (a.value.length > 256) {
		a.value = t.value.substring(0, 256)
	}
	EI("fmthint").innerHTML = "" + a.value.length + "/256"
}
function mothod(c) {
	var a = document.querySelectorAll('input[name$="method"]');
...
function update() {
	...
	r.format = encodeURIComponent(a.escapeJSON());
