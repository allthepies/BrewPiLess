...
BrewChart.prototype.showLegend = function (a, b) {
	var d = new Date(a);
	//Q(".beer-chart-legend-time").innerHTML = this.formatDate(d);
	Q(".beer-chart-legend-time").innerHTML = formatDate(d);
	Q(".chart-legend-row.beerTemp .legend-value").innerHTML = this.tempFormat(this.chart.getValue(b, 2));
	Q(".chart-legend-row.beerSet .legend-value").innerHTML = this.tempFormat(this.chart.getValue(b, 1));
	Q(".chart-legend-row.fridgeTemp .legend-value").innerHTML = this.tempFormat(this.chart.getValue(b, 3));
	Q(".chart-legend-row.fridgeSet .legend-value").innerHTML = this.tempFormat(this.chart.getValue(b, 4));
	Q(".chart-legend-row.roomTemp .legend-value").innerHTML = this.tempFormat(this.chart.getValue(b, 5));
	var c = parseInt(this.state[b]);
	if (!isNaN(c)) {
		Q('.chart-legend-row.state .legend-label').innerHTML = STATES[c].text
	}
};
BrewChart.prototype.hideLegend = function () {
	...
	Q(".beer-chart-legend-time").innerHTML = this.dateLabel;
...
BrewChart.prototype.tempFormat = function (y) {
	...
	if (isNaN(v))
		return "--";
	var a = this.celius ? "°C" : "°F";
	return parseFloat(v).toFixed(2) + a
};
...
BrewChart.prototype.toggleLine = function (a) {
	this.shownlist[a] = !this.shownlist[a];
	if (this.shownlist[a]) {
		Q("." + a + " .toggle").style.backgroundColor = Q(".chart-legend-row." + a).style.color;
		this.chart.setVisibility(this.chart.getPropertiesForSeries(a).column - 1, true)
	} else {
		Q("." + a + " .toggle").style.backgroundColor = "transparent";
		this.chart.setVisibility(this.chart.getPropertiesForSeries(a).column - 1, false)
	}
};
BrewChart.prototype.createChart = function () {
	...
	t.chart = new Dygraph(document.getElementById(t.cid), t.data, {
			...
			highlightCallback: function (e, x, a, b) {
				t.showLegend(x, b)
			},
...
BrewChart.Colors = ["rgb(240, 100, 100)", "rgb(41,170,41)", "rgb(89, 184, 255)", "rgb(255, 161, 76)", "#AAAAAA", "rgb(153,0,153)"];
BrewChart.Labels = ['Time', 'beerSet', 'beerTemp', 'fridgeTemp', 'fridgeSet', 'roomTemp'];
...
BrewChart.prototype.addResume = function (a) {
	this.ctime += a;
	this.anno.push({
		series: "beerTemp",
		x: this.ctime * 1000,
...
BrewChart.prototype.process = function (a) {
	var b = false;
	var t = this;
	for (var i = 0; i < a.length; ) {
		...
		var e = a[i++];
		if (c == 0xFF) {
			t.celius = (e & 0x10) ? false : true;
			var p = a[i++];
			p = p * 256 + a[i++];
			t.interval = p;
			t.starttime = (a[i] << 24) + (a[i + 1] << 16) + (a[i + 2] << 8) + a[i + 3];
			t.ctime = t.starttime;
			i += 4;
			t.data = [];
			t.anno = [];
			...
			var g = a[i++];
			var v = (f & 0x7F) * 256 + g;
			t.beerSet = (v == 0x7FFF) ? null : (v / 100)
		} else if (c == 0xFE) {
			if (t.lidx) {
				var h;
				for (h = t.lidx; h < t.numLine; h++)
					t.dataset.push(NaN);
...
var ControlChart = {
	...
	process: function (a) {
		...
		for (var i = 0; i < a.t.length; i++) {
			var c = new Date(b.getTime() + Math.round(a.t[i].d * 86400) * 1000);
			this.data.push([c, a.t[i].t])
		}
	},
	init: function (a, b) {
		var t = this;
		...
		t.chart = new Dygraph(document.getElementById(a), t.data, {
...
function formatDate(d) {
	...
	return d.toLocaleDateString(getLocale()) + " " + T(a) + ":" + T(b) + ":" + T(c)
};
...
var profileEditor = {
	...
	startDayChange: function () {
		var a = new Date(document.getElementById(this.C_startday_Id).value);
		if (isNaN(a.getTime())) {
	...
	deleteRow: function (i) {
		var a = document.getElementById("profile_t").getElementsByTagName("tr")[i];
	...
	showmenu: function (e, r) {
		...
		b.innerHTML = "Delete!";
		b.style.top = (e.clientY - 1) + "px";
		b.style.left = (e.clientX - 1) + "px";
	...
	dayChange: function (a) {
		if (a.innerHTML == "" || isNaN(a.innerHTML))
			a.innerHTML = a.saved;
		this.sortrow();
		this.markdirty(true)
	},
	tempChange: function (a) {
		if (a.innerHTML == "" || isNaN(a.innerHTML))
			a.innerHTML = a.saved;
		else {
			this.markdirty(true);
			ControlChart.update(this.getProfile())
		}
	},
	newrow: function (d, t, a) {
		var b = this;
		var c = document.createElement("tr");
		var f = document.createElement("td");
		f.className = "pday";
		f.innerHTML = (typeof d !== "undefined") ? d : 0;
		...
		g.className = "ptemp";
		g.innerHTML = (typeof t !== "undefined") ? t : 20;
	...
	datestr: function (a) {
		var b = new Date(this.sd.getTime() + Math.round(a * 86400) * 1000);
		return formatDate(b)
	},
	addRow: function () {
		var a = document.getElementById("profile_t").getElementsByTagName("tbody")[0];
		var b = a.getElementsByTagName("tr");
		var r = this.newrow();
		r.className = (b.length % 2) ? "odd" : "even";
		a.appendChild(r);
		this.markdirty(true)
	},
	init: function (s, d) {
	...
	sortrow: function () {
		...
		var g = Array.prototype.slice.call(f);
		g.sort(function (a, b) {
			var c = parseFloat(a.querySelector('td.pday').innerHTML);
			var d = parseFloat(b.querySelector('td.pday').innerHTML);
		...
		for (var i = 0; i < g.length; i++) {
			var h = g[i];
			var j = h.querySelector('td.pday').innerHTML;
			h.querySelector('td.pdaystr').innerHTML = this.datestr(parseFloat(j));
			h.className = (i % 2) ? "odd" : "even";
	...
	getProfile: function () {
		...
		for (var i = 0; i < b.length; i++) {
			var f = b[i];
			var g = parseFloat(f.querySelector('td.pday').innerHTML);
			var h = parseFloat(f.querySelector('td.ptemp').innerHTML);
			if (isNaN(g) || isNaN(h))
				return false;
			if (h > BrewPiSetting.maxDegree || h < BrewPiSetting.minDegree)
				return false;
			if (i == 0) {
				if (g != 0)
					return false
	...
	setTempUnit: function (u) {
		...
		var b = a.getElementsByTagName("tr");
		for (var i = 0; i < b.length; i++) {
			var c = b[i].querySelector('td.ptemp');
			var d = parseFloat(c.innerHTML);
			c.innerHTML = (u == 'C') ? F2C(d) : C2F(d)
		}
	}
};
var modekeeper = {
	...
	dselect: function (m) {
		...
		var a = document.getElementById(m + "-m").className.replace(/\snav-selected/, '');
		d.className = a;
...
function saveprofile() {
	...
	BWF.save(BWF.BrewProfile, a, function () {
...
function updateTempUnit(u) {
	var a = document.getElementsByClassName("t_unit");
...
function onload() {
	...
	function complete() {
		...
		invoke({
			...
			success: function (d) {
				...
				if (a.tempUnit != BrewPiSetting.tempUnit) {
					updateTempUnit(a.tempUnit);
					profileEditor.setTempUnit(a.tempUnit)
				}
				BrewPiSetting = a;
				initComp()
			}
	...
	BWF.load(BWF.BrewProfile, function (d) {
		...
		ControlChart.init("tc_chart", p);
		complete()
	}, function (e) {
		profileEditor.init([], new Date());
		ControlChart.init("tc_chart");
		complete()
	})
}
function s_ajax(b) {
	...
	c.ontimeout = function () {
		if (typeof b["timeout"] != "undefined")
			b.timeout();
		else
			c.onerror(-1)
	},
	c.onerror = function (a) {
		if (typeof b["fail"] != "undefined")
			b.fail(a)
	};
...
var BChart = {
	...
	url: 'chart.php',
	toggle: function (a) {
		this.chart.toggleLine(a)
	},
	reqdata: function () {
		var t = this;
		var c = 'offset=' + t.offset;
		if (typeof t.startOff != "undefined" && t.startOff !== null)
			c = c + "&index=" + t.startOff;
		...
		d.onload = function (e) {
			...
			var b = t.chart.process(a);
			if (b) {
				t.offset = a.length;
				t.startOff = d.getResponseHeader("LogOffset");
				getLogName();
